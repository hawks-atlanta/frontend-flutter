...
final accountFormPassProvider = StateNotifierProvider.autoDispose<
  ...
  final accountPasswordCallback = ref
      .watch(accountPassProvider.notifier)
      .changePassword; //ref.watch(authProvider.notifier).changePassword;
  return AccountPasswordNotifier(
      accountPasswordCallback: accountPasswordCallback);
});
...
class AccountPasswordNotifier extends StateNotifier<AccountPasswordFormState> {
  ...
  _touchEveryField() {
    ...
    state = state.copyWith(
        ...
        isValid: Formz.validate([oldpassword, newpassword, repeatNewPassword]));
  }
}
